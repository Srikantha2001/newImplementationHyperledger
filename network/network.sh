...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-mngorg.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/mngorg/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating mngorg Identities"
  ...
  infoln "Generating CCP files for mngorg "
...
# call the script to create the channel, join the peers of mngorg ,
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.mngorg.example.com docker_peer1.mngorg.example.com docker_peer2.mngorg.example.com docker_peer3.mngorg.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/mngorg/msp organizations/fabric-ca/mngorg/tls-cert.pem organizations/fabric-ca/mngorg/ca-cert.pem organizations/fabric-ca/mngorg/IssuerPublicKey organizations/fabric-ca/mngorg/IssuerRevocationPublicKey organizations/fabric-ca/mngorg/fabric-ca-server.db'
