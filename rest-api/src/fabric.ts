...
/**
 * Creates an in memory wallet to hold credentials for an MngOrg user
 *
...
export const createWallet = async (): Promise<Wallet> => {
  ...
  const mngorgIdentity = {
    credentials: {
      certificate: config.certificateMngOrg,
      privateKey: config.privateKeyMngOrg,
    },
    mspId: config.mspIdMngOrg,
    type: 'X.509',
  };

  await wallet.put(config.mspIdMngOrg, mngorgIdentity);
