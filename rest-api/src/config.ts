 ...
 * The MngOrg MSP ID
 */
export const mspIdMngOrg = env
  .get('HLF_MSP_ID_MNGORG')
  .default(`${MNGORG}MSP`)
  .example(`${MNGORG}MSP`)
  .asString();
...
/**
 * The MngOrg connection profile JSON
 */
export const connectionProfileMngOrg = env
  .get('HLF_CONNECTION_PROFILE_MNGORG')
  .required()
  .example(
    '{"name":"test-network-mngorg","version":"1.0.0","client":{"organization":"MngOrg" ... }'
  )
  .asJsonObject() as Record<string, unknown>;

/**
 * Certificate for an MngOrg identity to evaluate and submit transactions
 */
export const certificateMngOrg = env
  .get('HLF_CERTIFICATE_MNGORG')
  .required()
  .example('"-----BEGIN CERTIFICATE-----\\n...\\n-----END CERTIFICATE-----\\n"')
  .asString();

/**
 * Private key for an MngOrg identity to evaluate and submit transactions
 */
export const privateKeyMngOrg = env
  .get('HLF_PRIVATE_KEY_MNGORG')
  .required()
...
/**
 * API key for MngOrg
 * Specify this API key with the X-Api-Key header to use the MngOrg connection profile and credentials
 */
export const mngorgApiKey = env
  .get('MNGORG_APIKEY')
  .required()
  .example('123')
  .asString();
